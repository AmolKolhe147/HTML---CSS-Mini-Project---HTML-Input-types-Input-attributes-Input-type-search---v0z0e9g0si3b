...
3-> is for Main image on first page.
...
img tag  - first image - river
div caption is used for heading of the image <h3> and some details about image is saved in <p> tag.
...
-> heading <h3> - Get in Touch
-> <p>  - Email or call us to set up a consult
-> under <p> tag we have added email in <strong>  tag - contact@acmephotos.test
-> Inside <p> tag Phone no is added in <strong> tag.
...
#landing image id <div> - background image is saved using backgroung url property,background position, background size, background repeat,margin,flex,height these properties set for background image.
...
<div> caption class for imaages - padding,text-align properties set.

<footer> tag - text-align,padding,margin and color properties are set.

heading <h3> tag of footer added some properties - font-size and margin-bootom.

at the end we have applied some media queries on <div> landing under <main> tag. we have designed mobile first approach at the starting and then applied media queries.
